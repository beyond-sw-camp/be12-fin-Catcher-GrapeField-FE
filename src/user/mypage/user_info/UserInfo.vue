...
onMounted(async () => {
  try {
    ...
    const profileImg = userStore.profileImg || '';
...
<template>
  <div class="w-full max-w-2xl mx-auto px-8 py-6 text-sm font-['Inter'] space-y-8">
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="relative">
          <!-- 수정 모드: 업로드 가능 -->
          <label v-if="isEditMode" class="cursor-pointer block" for="profile-upload">
          ...
          <!-- 삭제 버튼 (오른쪽 하단에 X 아이콘) -->
          <button v-if="isEditMode && user.profileImg" class="absolute -right-1 -bottom-1 w-5 h-5 bg-white border border-gray-300 rounded-full text-xs flex items-center justify-center hover:bg-red-500 hover:text-white"
                  title="삭제"
                  @click="user.profileImg = ''">
            ×
          </button>

          <!-- input hidden -->
          <input id="profile-upload" accept="image/*" class="hidden" type="file" @change="onProfileChange"/>
        </div>
        ...
        <div class="flex flex-col gap-1 w-full">
          ...
          <input v-else v-model="user.name" class="w-full border rounded px-3 py-2" type="text"/>
          <p v-if="isEditMode" class="text-gray-500 text-xs">클릭하여 프로필 사진 변경</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-else class="flex gap-2">
          <input v-model="user.phone" class="flex-1 border rounded px-3 py-2" type="tel"/>
    ...
    <div v-if="isPasswordConfirming" class="space-y-4 pt-4 border-t">
      ...
      <button class="w-full bg-purple-600 text-white py-2 rounded font-semibold" @click="confirmPassword">
        확인
      </button>
    </div>

    <!-- 하단 버튼 -->
    <div v-if="!isEditMode && !isPasswordConfirming" class="pt-6 flex justify-between items-center">
      <!-- 왼쪽 버튼들 묶음 -->
  ...
  <div v-if="isEditMode" class="pt-6 justify-center text-center">
    <button class="px-3 py-2 bg-purple-600 hover:bg-purple-700 text-white py-3 rounded font-semibold"
            @click="saveChanges">
      변경사항 저장
    </button>
  </div>
