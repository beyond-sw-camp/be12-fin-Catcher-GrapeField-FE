  ...
  <div class="w-full min-h-screen bg-white px-4 sm:px-10 md:px-20 pt-28">
    <div class="max-w-screen-xl mx-auto">
      <CalendarHeader :year="year" :month="month" title="나의 캘린더" subtitle="예매한 공연과 관심 있는 일정을 한눈에 확인하세요."
        :showLegend="false" @prev="prevMonth" @next="nextMonth" />
      ...
      <div>
        ...
        <!-- 일정 추가 모달 -->
        <AddSchedule :isOpen="showModal" :initialDate="selectedDate" @close="showModal = false"
...
function handleScheduleSubmit(newSchedule) {
  console.log('저장된 일정:', newSchedule)
  showModal.value = false
  // ✅ JSON에 추가하거나 상태 업데이트 등 여기서 처리
}
...
function handleDateClick(date) {
  selectedDate.value = date                     // ⬅️ 모달용
  showModal.value = true                        // ⬅️ 모달 열기
  selectedBooking.value = bookingInfo.value.find(item => item.date === date) || null // ⬅️ 예매 카드용
}
